...
/* Planten Databank – client-side (GitHub Pages)
   - Fyto: toont 3 extra kolommen in resultaten wanneer aangevinkt:
     1) Comments on phytoremedial effectiveness
     2) Continent-Country-City-Site
     3) Reference (author, year, doi)
   - Layer matching: Latijnse naam (primair) + Nederlandse naam (fallback)
   - Geen Ecoflora prijsbadge in resultaten
   - Klik op een rij om foto’s + fyto-details te tonen in rechter “drawer”
*/
...
// Regionale district filename convention
...
function plantFromRow(row){
  ...
  // dynamic facets: any kenmerk_* not in base keys
...
function matchesFacets(p){
  for(const [k, wantedSet] of STATE.selected.facets.entries()){
    ...
    if(!vals.some(v=>wantedSet.has(v))) return false;
  }
  return true;
}
    ...
    for(const p of STATE.plants){
      ...
      if(wanted && wanted!=="ALLE"){
        if(code && norm(code).toLowerCase()===norm(wanted).toLowerCase()){
          p.layers.bobo=true; p.boboCode=code||wanted;
        }
      }else{
        p.layers.bobo=true; p.boboCode=code||null;
      }
    }
  }
...
/* Drawer with images */
function safeSlugLatin(latin){
  return keyLatin(latin).replaceAll(" ", "_").replaceAll("/", "_");
}
function buildImageCandidates(latin){
  const slug = safeSlugLatin(latin);
  const exts = ["jpg","jpeg","png","webp"];
  const names = [];
  for(const ext of exts){
    names.push(`assets/images/${slug}.${ext}`);
    for(let i=1;i<=5;i++) names.push(`assets/images/${slug}_${i}.${ext}`);
  }
  return names;
}
async function loadFirstImages(latin, max=4){
  const candidates = buildImageCandidates(latin);
  const found = [];
...
function openDrawer(plant){
  ...
  imgHost.innerHTML = `<div class="hint">Zoeken naar foto…</div>`;
  loadFirstImages(plant.latin, 4).then(urls=>{
    if(!urls.length){
      imgHost.innerHTML = `<div class="hint">Geen foto gevonden. Upload naar <code>assets/images/${safeSlugLatin(plant.latin)}.jpg</code> (of _1, _2…).</div>`;
      return;
    }
    if(urls.length===1){
      imgHost.innerHTML = `<img src="${urls[0]}" alt="${plant.latin}">`;
      return;
    }
    const main=urls[0], thumbs=urls.slice(1);
    imgHost.innerHTML = `
      <img id="drawerMainImg" src="${main}" alt="${plant.latin}">
      <div style="display:flex;gap:8px;padding:10px;flex-wrap:wrap;border-top:1px solid var(--line);background:#0b0f10">
        ${thumbs.map(u=>`<img class="thumb" src="${u}" style="width:72px;height:48px;object-fit:cover;border-radius:10px;border:1px solid var(--line);cursor:pointer" alt="">`).join("")}
      </div>
    `;
    const mainEl = $("#drawerMainImg");
    imgHost.querySelectorAll(".thumb").forEach((el, idx)=>{
      el.addEventListener("click", ()=>{ mainEl.src = thumbs[idx]; });
    });
  });
...
function wire(){
  ...
  $("#exportCsv").addEventListener("click", ()=>{
    ...
    downloadText(`planten_${STATE.selected.typology}_${STATE.selected.subtype}.csv`, csv);
  });
}
...
async function init(){
  ...
  // region districts dropdown
  const districtSel=$("#district"); districtSel.innerHTML="";
  const opt0=document.createElement("option"); opt0.value=""; opt0.textContent="Kies district…";
  ...
  const optG=document.createElement("option"); optG.value=""; optG.textContent="Kies groep…";
  bg.appendChild(optG);
