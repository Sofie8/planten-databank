/* Planten Databank – client-side (GitHub Pages)
   ...
   - Foto's in assets/images/<foto_id>.jpg of .png
   - Klik op Nederlandse naam opent drawer met foto's (onafhankelijk van fytoremediatie)
   - Fyto: 3 extra kolommen in tabel wanneer Fyto-laag actief
   - Geen Ecoflora-prijs badge
...
const STATE = {
  ...
  selected: {
    ...
    district: "", regionMode: "paint",
    boboGroup: "", boboCode: "ALLE",
    facets: new Map()
  },
  table: { extraCols: [] }
};
...
function sheetToJson(wb, sheetName){ return XLSX.utils.sheet_to_json(wb.Sheets[sheetName], {defval:""}); }
...
function layerFiles(layerKey, typology){
  ...
  const names=(STATE.config?.layers?.[{ klimaat:"klimaatbomenlijst", amber:"amberlijst" }[layerKey]] ?? []).map(norm).filter(Boolean);
  return names.map(n=>`data/layers/${n}.xlsx`);
}
...
async function loadTypologyPlants(typology, subtype){
  ...
  for(const file of files){
    if(STATE.loaded.typologies.has(file)){ all.push(...STATE.loaded.typologies.get(file)); continue; }
...
function fillSelect(selectEl, values, includeAll=true){
  ...
  selectEl.innerHTML="";
  const add=(v,label)=>{ const opt=document.createElement("option"); opt.value=v; opt.textContent=label??v; selectEl.appendChild(opt); };
...
function matchesBaseFilters(p){
  ...
  return ((st==="alle")||p.traits.bodemtype.includes(st)) &&
...
function matchesAll(p){ return matchesBaseFilters(p) && matchesFacets(p) && matchesRegionMode(p); }
  ...
  if(STATE.selected.layers.bobo){
    ...
    for(const p of STATE.plants){
      ...
      if(wanted && wanted!=="ALLE"){
        if(code && norm(code).toLowerCase()===norm(wanted).toLowerCase()){ p.layers.bobo=true; p.boboCode=code||wanted; }
      }else{ p.layers.bobo=true; p.boboCode=code||null; }
    }
  }
...
/* Drawer + image carousel (foto_ids) – jpg/png only */
function urlForconst IMAGE_BASE_URL = "https://pub-bb204453b9b642598d8514f7ac4f68be.r2.dev";
...
function openDrawer(plant){
  ...
  if(!ids.length){
    ...
    resolveFotoUrls(ids, 80).then(urls=>{
      if(!urls.length){
        imgHost.innerHTML = `<div class="hint">Geen foto’s gevonden in <code>assets/images</code> voor deze foto_ids.</div>`;
        return;
      }
      let idx = 0;
...
function render(plants){
  ...
  for(const p of filtered){
    ...
    const tdSoil=document.createElement("td"); tdSoil.textContent=(p.traits.bodemtype||[]).join(", ")||"—"; tr.appendChild(tdSoil);
    const tdMoist=document.createElement("td"); tdMoist.textContent=(p.traits.bodemvocht||[]).join(", ")||"—"; tr.appendChild(tdMoist);
    const tdPh=document.createElement("td"); tdPh.textContent=(p.traits.zuur||[]).join(", ")||"—"; tr.appendChild(tdPh);
    ...
    for(const t of badgesForPlant(p)){
      const s=document.createElement("span"); s.className="badge ok"; s.textContent=t; wrap.appendChild(s);
    }
...
function humanizeFacetKey(k){ return norm(k).replace(/^kenmerk_/i,"").replaceAll("_"," "); }

function renderExtraFilters(plants){
  const host=$("#extraFiltersList"); if(!host) return;
  host.innerHTML="";
  const counts=computeFacetOptions(plants);
  const keys=uniqSorted(Array.from(counts.keys()));
  if(!keys.length){ host.innerHTML = `<div class="hint">—</div>`; return; }
  ...
  keys.forEach((k, idx)=>{
    ...
    const header=document.createElement("div"); header.className="facetHeader";
    header.innerHTML = `<div><div class="facetTitle">${humanizeFacetKey(k)}</div><div class="facetMeta">${entries.length} opties</div></div><div>▾</div>`;
    header.addEventListener("click", ()=>toggleFacetPanel(idx));
    const body=document.createElement("div"); body.className="facetBody"; body.id=`facetBody_${idx}`;
    const opts=document.createElement("div"); opts.className="facetOptions";
...
function wire(){
  ...
  $("#typology").addEventListener("change", async (e)=>{
    ...
    for(const s of subs){ const opt=document.createElement("option"); opt.value=s; opt.textContent=s; subSel.appendChild(opt); }
  ...
  $("#district").addEventListener("change", async (e)=>{ STATE.selected.district=e.target.value; await applyLayers(); render(STATE.plants); });

  const paint=$("#regionModePaint"), filt=$("#regionModeFilter");
  if(paint && filt){
  ...
  $("#boboCode").addEventListener("change", async (e)=>{ STATE.selected.boboCode=e.target.value; await applyLayers(); render(STATE.plants); });
...
async function init(){
  ...
  for(const t of typologyOptions()){ const opt=document.createElement("option"); opt.value=t; opt.textContent=t; typSel.appendChild(opt); }
  STATE.selected.typology=typologyOptions()[0]; typSel.value=STATE.selected.typology;
  ...
  const districtSel=$("#district"); districtSel.innerHTML="";
  const opt0=document.createElement("option"); opt0.value=""; opt0.textContent="Kies district…"; districtSel.appendChild(opt0);
  for(const d of STATE.options.region.districts){ const opt=document.createElement("option"); opt.value=d; opt.textContent=d; districtSel.appendChild(opt); }
  STATE.selected.district="";

  const bg=$("#boboGroup"); bg.innerHTML="";
  const optG=document.createElement("option"); optG.value=""; optG.textContent="Kies groep…"; bg.appendChild(optG);
  for(const g of STATE.options.bobo.groups){ const opt=document.createElement("option"); opt.value=g; opt.textContent=g; bg.appendChild(opt); }
